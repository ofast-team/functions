...
export async function submit(req: Request, res: Response) {
    ...
    let date = new Date()
    addDoc(collection(db, 'Submissions'), { tokens: tokens, pending: true, date: date, problem_id: -1, total_cases: tokens.length, verdict: 1 })
        .then((id) => {
        ...
        .then(async (a) => {
            if (a.exists()) {
                ...
                if (info.pending == true) {
                    ...
                    let url = judge_url + "/submissions/batch?tokens=" + token_string + "&fields=status_id,time"
                    ...
                    .then(() => {
                        return res.status(200).json({date: new_object.date, problem_id: new_object.problem_id, verdict: verdict, verdict_list: verdict_list, passed_cases: pass_count, total_cases: new_object.total_cases})
                    })
                    .catch((err) => {
                        return res.status(500).json({error: err})
                    })
                    ...
                    return res.status(200).json({date: info.date, problem_id: info.problem_id, verdict: info.verdict, verdict_list: info.verdict_list, passed_cases: info.passed_cases, total_cases: info.total_cases})
                }
            } else {
